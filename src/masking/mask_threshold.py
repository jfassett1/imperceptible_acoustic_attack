...
def parallel_compute_th(PSD, barks, ATH, freqs, test, num_workers=4):
    ...
    with Pool(processes=num_workers) as pool:  # Create the pool once
        for samp in range(PSD.shape[0]):
            args_list = [(PSD[samp, :, i], barks, ATH, freqs, test) for i in range(PSD.shape[2])]
            
            if num_workers > 0:
                theta_i = pool.starmap(compute_th, args_list)  # Run in parallel
            else:
                theta_i = [compute_th(*args) for args in args_list]  # Fallback to sequential execution
...
def generate_th_batch(audio, fs, window_size=2048,test=1,num_workers=0):
    ...
    if num_workers == 0:
        ...
        theta_xs = parallel_compute_th(PSD, barks, ATH, freqs, test, num_workers=4)
