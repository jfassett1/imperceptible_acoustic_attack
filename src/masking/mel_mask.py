...
def compute_mel_bin_centers(mel_filter_bank, sr, n_fft):
    ...
    mel_filter_bank_np = mel_filter_bank.cpu().numpy() if torch.is_tensor(mel_filter_bank) else mel_filter_bank
    centers = np.array([
...
mel_frequencies = torch.from_numpy(compute_mel_bin_centers(mel_filter_bank, SAMPLING_RATE, N_FFT))

def log_mel_spectrogram_max(
    ...
    This allows us to pass in a max value for normalization.
